...
# ================= SIMPLE HILL CLIMBING (FIRST IMPROVING MOVE) =================
def simple_hill_climbing(start, goal):
    """
    Simple Hill Climbing: at each step, move to the FIRST neighbor 
    that improves h (Manhattan distance). If none improves, stop.
    Returns (path_moves, list_of_states).
    """
    current = start
    path = []
    states = [current]
    ...
    while current != goal:
        ...
        found_better = False
        # Try neighbors in a fixed order (U, D, L, R) - first improving move
        for nxt, mv in neighbors(current):
            if nxt in visited:
                continue
            h_val = manhattan(nxt, goal)
            if h_val < current_h:
                # Move to this neighbor immediately
                path.append(mv)
                states.append(nxt)
                visited.add(nxt)
                current = nxt
...
class Puzzle:
    def __init__(self, root):
        ...
        # Now put everything into scrollable_frame
        ...
        buttons = [
            ...
            ("A*", self.solve_astar),
            ("Hill Climbing", self.solve_hill)
        ]

        for i, (txt, cmd) in enumerate(buttons):
            b = tk.Button(bf, text=txt, width=14, height=1, font=("Arial",10,"bold"),
                          bg=BTN, fg="white", activebackground=BTN_HOVER, relief="flat",
                          cursor="hand2", command=cmd)
            b.grid(row=i//2, column=i%2, padx=6, pady=6, sticky="nsew")

        reset_btn = tk.Button(bf, text="RESET", width=30, height=1, font=("Arial",10,"bold"),
                              bg="#ff5f5f", fg="white", relief="flat", cursor="hand2",
                              command=self.reset)
        reset_btn.grid(row=4, column=0, columnspan=2, padx=6, pady=10, sticky="nsew")
        ...
        self.info = tk.Label(right, text="g(n)=0    h(n)=0    f(n)=0",
                             font=("Consolas",14,"bold"), bg=FRAME, fg=INFO)
    ...
    def animate(self, path, states, algo):
        ...
        self.write("")
        goal = self.read(self.goal)
        uses_heuristic = algo in ["GREEDY", "A*", "Hill Climbing"]
    ...
    def solve_astar(self):
        self.solve("A*", astar)
    def solve_hill(self):
        self.solve("Hill Climbing", simple_hill_climbing)
